...
if (!empty($comp['M1_temp'])) {
	...
	$sql = "insert into COMPONENT ";
	$sql .= "(id, comp_id,description, prep_type_id, M1_check_id, M1_time, M1_chef_id,finished,shelf_life_days,expiry_date) ";
	$sql .= "values (null,".$comp_id.",'".$description."',".$prep_type.",".$userID.",now(),".$M1_chef_id.",now(),".$shelf_life_days;
	$sql .= ",DATE_ADD(now(), INTERVAL ".$shelf_life_days." DAY))";
}
